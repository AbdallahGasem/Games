...
class five_x_five_board : public Board 
{
protected:
    int X_noof_wins = 0 ;
    int O_noof_wins = 0 ;
    char charchter ;
public:
    five_x_five_board() ; //done
    bool update_board (int x, int y, char mark); // done
    void display_board(); // done
    bool is_winner(); // done
    void final_winner(); // done
    void detectwiningmark(char x) ; // done
    bool is_draw();
    bool game_is_over();
};
...
//This class represents an AI player that chooses the best possible move
class connectfourAI : public Player , Board
{
protected:
int dimension;
public:
connectfourAI(char symbol , int dimension);
   void get_move(int &x, int &y);
